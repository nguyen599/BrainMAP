    ...
    def forward(self, batch):
        ...
        if self.use_exp_edges:
            ...
            edge_attr_sets.append(
                self.exp_edge_attr(
                    torch.zeros(exp_edges.shape[1], dtype=torch.long).to(device)
                )
            )
        ...
        if self.num_virt_node > 0:
            ...
            for idx in range(self.num_virt_node):
                global_h.append(
                    self.virt_node_emb(
                        torch.zeros(num_graphs, dtype=torch.long).to(device) + idx
                    )
                )
                virt_edge_index = torch.cat(
                    [
                        torch.arange(num_node).view(1, -1).to(device),
                ...
                virt_edge_attrs.append(
                    self.virt_edge_in_emb(
                        torch.zeros(virt_edge_index.shape[1], dtype=torch.long).to(
                            device
                        )
                        + idx
                    )
            ...
            edge_types.append(
                torch.zeros(batch.virt_edge_index.shape[1], dtype=torch.long) + 2
            )
