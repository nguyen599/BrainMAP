...
def add_dist_features(data, max_n, is_undirected, cutoff=None):
    ...
    dist_factors = list(
        map(partial(bfs_shortest_path, G=G, max_n=max_n, cutoff=cutoff), list(range(n)))
    )
...
def incidence_matrix(
    senders: np.ndarray, receivers: np.ndarray, n: Optional[int] = None
) -> Any:
...
def effective_resistance_embedding(
    data: Data,
    MaxK: int,
    weights: Optional[np.ndarray] = None,
    ...
    data.er_emb = torch.nn.functional.pad(
        embedding, (0, MaxK - embedding.shape[1])
    ).float()
    return data


def effective_resistances_from_embedding(
    data: Data, normalize_per_node: bool = False
) -> Any:
