...
def setup_random_split(dataset):
    ...
    val_index, test_index = next(
        ShuffleSplit(
            train_size=split_ratios[1] / (1 - split_ratios[0]), random_state=cfg.seed
...
def set_dataset_splits(dataset, splits):
    ...
    if task_level == "node":
        ...
        max_in_splits = [max(split) for split in splits]
        if len(dataset) == max(max_in_splits) + 1:
            batch_data = Batch.from_data_list(dataset)
            train_mask = batch_data.batch <= max_in_splits[0]
            val_mask = (batch_data.batch > max_in_splits[0]) & (
                batch_data.batch <= max_in_splits[1]
            )
            test_mask = batch_data.batch > max_in_splits[1]
            dataset.data = batch_data
            set_dataset_attr(dataset, "train_mask", train_mask, len(train_mask))
            set_dataset_attr(dataset, "val_mask", val_mask, len(val_mask))
            set_dataset_attr(dataset, "test_mask", test_mask, len(test_mask))
