...
from graphgps.transform.dist_transforms import (
    add_dist_features,
    add_reverse_edges,
    add_self_loops,
...
def load_dataset_master(format, name, dataset_dir):
    ...
    if format.startswith("PyG-"):
        ...
        if pyg_dataset_id == "GNNBenchmarkDataset":
            ...
            if name == "photo" or name == "computers":
                pre_transform_in_memory(
                    dataset, partial(generate_splits, g_split=cfg.dataset.split[0])
                )
            ...
            dataset = Planetoid(
                dataset_dir, name, split="random", train_percent=cfg.prep.train_percent
            )
    ...
    # GraphGym default loader for Pytorch Geometric datasets
        ...
        if name.startswith("ogbg"):
            ...
            # if cfg.test_with_subgraph:
            #     dataset = dataset.index_select(torch.arange(1000))
    ...
    for key, pecfg in cfg.items():
        if (
            key.startswith("posenc_")
            and pecfg.enable
            and (not key.startswith("posenc_ER"))
        ):
    ...
    if cfg.posenc_ERN.enable or cfg.posenc_ERE.enable:
        ...
        MaxK = max(
            [
                min(
                    ...
                    math.ceil(
                        8 * math.log(data.num_edges) / (cfg.posenc_ERN.accuracy**2)
                    ),
                )
                for data in dataset
            ]
        ...
        pre_transform_in_memory(
            dataset,
            partial(effective_resistances_from_embedding, normalize_per_node=False),
            show_progress=True,
        )
